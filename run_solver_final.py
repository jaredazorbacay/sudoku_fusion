...
OUTPUT_FILE = "[25x25]initialTemp-3.625.csv"
TIMEOUT = "120"
...
def main():
    ...
    print(f"Running {len(files)} instance files with {NUM_THREADS} threads")

    results = []

    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:

        futures = [executor.submit(process_instance, f) for f in files]
